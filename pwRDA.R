...
#'   \item \code{rda.0}: full model cca object,  which is described separately in \code{\link[vegan]{cca.object}}
#'   \item \code{rda.pw}: pw model cca object, which is described separately in \code{\link[vegan]{cca.object}}
#' }
#' @examples
#' data(sim1)
#' sim1.o<-OrdData(sim1)
...
pwRDA<-function(x.ord,y.ord,BPs, n.rand=99, progress=T )
{
  ...
  for( b in 1:n.rand)
  {
    ...
    if(progress==T){message(paste("Progress", (round(b/n.rand*100)), "%"))} }
...
pwRDA.source <-function(x.ord,y.ord, BPs) {
  ...
  coord<-scores(rda.pw,display="bp")
  bew.bp<-t(cor(coord,t(cor(x.ord,Xb))))
  rda.pw$CCA$biplot<-bew.bp
  #rda.pw$CCA$biplot<-cor(x.ord, Z)
  Ypred.pw = Xbc%*%B.pw
  Yres <- Yc - Ypred.pw
  TSS.pw= sum((Yc)^2)
